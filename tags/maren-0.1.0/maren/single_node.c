...
MarenSingleNode*
maren_single_node_ctor( void* where,
			MarenSingleCheckFn check,
			void* data,
			void (*del_data)(void*) )
{
  MarenSingleNode* ret = where ? where : malloc( sizeof(MarenSingleNode) );

  if ( ret ) {
    maren_inner_node_ctor( (MarenInnerNode*)ret, MAREN_NT_SINGLE );
    ret->check = check;
    ret->data = data;
...
void
maren_single_node_dtor( MarenSingleNode* node )
{
  if ( node ) {
